...
public class FieldCommandImpl implements CriteriaCommand {
	...
	public Criteria execute(Criteria criteria) {
    	...
    	if("LT".equals(operation)){
			...
			Long[] ls=new Long[s.length];
			 for (int i = 0; i < ls.length; i++) { //把值赋予数组
		            ls[i] = Long.parseLong(s[i])  ;
		        }
			criteria.add(Restrictions.in(property, ls));
		}else if("SIN".equals(operation)){
			String[]s = ((String)value).split("[,]"); 
			criteria.add(Restrictions.in(property, s));
		}else{
