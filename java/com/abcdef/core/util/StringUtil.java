...
public class StringUtil {
	...
	public static String toString(Object obj, boolean autoQuote) {
		...
		if (autoQuote
				&& sb.length() > 0
				&& !((sb.charAt(0) == '[' && sb.charAt(sb.length() - 1) == ']') || (sb.charAt(0) == '{' && sb.charAt(sb
						.length() - 1) == '}'))) {
			sb.insert(0, "[").append("]");
		}
		return sb.toString();
	...
	public static StringBuilder formatMsgNoBracket(String msgWithFormat, Object... args) {
		return formatMsg(new StringBuilder(msgWithFormat), true, args);
	}
...
//		String result=htmlEntityToString(str);
		...
		String vm="abc.em";
		System.out.println(StringUtil.stringToHtmlEntity(vm));
	...
	/**
	 * 传入一个json格式，返回一个map对象
	 * @param mapString 格式如任务1:3|任务2:4
	 * @return
	 */
