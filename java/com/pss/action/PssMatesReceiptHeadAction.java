...
public class PssMatesReceiptHeadAction extends BaseAction {
	...
	public String multiDel() {
		...
		if (ids != null) {
			for (String id : ids) {
				try {
					pssMatesReceiptHeadService.remove(new Long(id));
	...
	public String get() {
		...
		PssMatesReceiptHead pssMatesReceiptHead = pssMatesReceiptHeadService.get(new Long(id));

		Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().create();
		// 将数据转成JSON格式
		StringBuffer sb = new StringBuffer("{success:true,data:");
		sb.append(gson.toJson(pssMatesReceiptHead));
		sb.append("}");
